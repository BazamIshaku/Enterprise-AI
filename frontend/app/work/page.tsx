...
export default function WorkPage() {
  ...
  const filterPickerRef = useRef<HTMLDetailsElement>(null);
  ...
  const selectedFilterEmployee = employees.find((item) => item.id === employeeFilter);
  const filteredEmployees = useMemo(() => { const query = filterEmployeeQuery.trim().toLowerCase(); return employees.filter((item) => !query || [item.name, item.role, item.department].some((value) => value.toLowerCase().includes(query))); }, [employees, filterEmployeeQuery]);
  const modalEmployees = useMemo(() => { const query = modalEmployeeQuery.trim().toLowerCase(); return activeEmployees.filter((item) => !query || [item.name, item.role, item.department].some((value) => value.toLowerCase().includes(query))); }, [activeEmployees, modalEmployeeQuery]);
  ...
  return <WorkspaceShell><section className={styles.heading}><div><p>AI WORK DESK</p><h1>Work together, with oversight</h1><span>Assign company work, follow progress, communicate, and approve every result.</span></div><button onClick={() => { setModalEmployeeId(employeeFilter === "all" ? "" : employeeFilter); setOpen(true); }}>+ Assign task</button></section>
    ...
    <section className={styles.finder}><label><span>Find a task</span><input type="search" value={taskQuery} onChange={(event) => setTaskQuery(event.target.value)} placeholder="Search tasks, roles, or departments" /></label><div className={styles.filterPicker}><span>AI employee</span><details ref={filterPickerRef}><summary><b>{selectedFilterEmployee ? initials(selectedFilterEmployee.name) : "AI"}</b><span><strong>{selectedFilterEmployee?.name ?? `All AI employees (${employees.length})`}</strong><small>{selectedFilterEmployee ? `${selectedFilterEmployee.role} · ${selectedFilterEmployee.department}` : "View work across the company"}</small></span><i>⌄</i></summary><div className={styles.filterMenu}><input aria-label="Search AI employees" value={filterEmployeeQuery} onChange={(event) => setFilterEmployeeQuery(event.target.value)} placeholder="Search employee, role, or department" type="search"/><button className={employeeFilter === "all" ? styles.chosenEmployee : ""} type="button" onClick={() => { setEmployeeFilter("all"); filterPickerRef.current?.removeAttribute("open"); }}><b>AI</b><span><strong>All AI employees</strong><small>{employees.length} employees across all departments</small></span>{employeeFilter === "all" && <i>✓</i>}</button>{filteredEmployees.map((item) => <button className={employeeFilter === item.id ? styles.chosenEmployee : ""} type="button" key={item.id} onClick={() => { setEmployeeFilter(item.id); filterPickerRef.current?.removeAttribute("open"); }}><b>{initials(item.name)}</b><span><strong>{item.name}</strong><small>{item.role} · {item.department}</small></span>{employeeFilter === item.id && <i>✓</i>}</button>)}{!filteredEmployees.length && <p>No employees match your search.</p>}</div></details></div></section>
